...
- Live verification accepted - 10 September 2026: the user confirmed real-MySQL verification of commit 1066148. Local SUPER_ADMIN login succeeded. GET `/api/v1/admin/technicians` returned HTTP 200 with technicianProfileId values, and GET `/api/v1/admin/customers` returned HTTP 200 with customerProfileId values. Both returned the expected page envelope. This supersedes the earlier unavailable-backend limitation; these are user-confirmed observations, not an independent replay by the assistant. No response bodies, credentials, tokens, headers or personal contact values are copied into this record. This confirms directory reads and selection identifiers, not an additional workflow write test.
...
- Next: separately authorized Admin Portal workflow migration using the verified directory routes. Implementation 1066148 and this documentation follow-up are ready for an explicitly authorized push of main; nothing was pushed. Production OTP provider and other previously deferred capabilities remain unchanged.
- Documentation follow-up only: existing 111-test and Maven package passes remain the validation baseline; no tests were rerun, application code changed or database accessed for this record. README.md and the Postman environment remain unstaged and preserved.
