...
@RestControllerAdvice(basePackages = "com.valor.notifications")
class NotificationErrorHandler {
    @ExceptionHandler(NotificationException.class)
    ResponseEntity<ApiResponse<Object>> notification(NotificationException error) { return response(error.status, error.getMessage()); }
    ...
    ResponseEntity<ApiResponse<Object>> invalid(Exception ignored) { return response(400, "Invalid notification request"); }
    @ExceptionHandler({DataIntegrityViolationException.class, PessimisticLockingFailureException.class})
    ResponseEntity<ApiResponse<Object>> conflict(Exception ignored) { return response(409, "Notification conflict"); }
